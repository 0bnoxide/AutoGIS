...
| [TraceSurvey123Provenance](docs/survey123-submission-provenance.md) | — | `envmon trace-survey123` | DRAFT: device SQLite → complete hosted/client snapshots, exact identity evidence and progress counts; optional live read-only layers (ADR-0136) |
...
| [TransformLandXMLSurface](autogis/core/envmon/landxml_transform.py) | `envmon transform-landxml` | ArcGIS Project-style transformation of one selected LandXML TIN from a geographic or projected authority-coded CRS to a projected EPSG CRS, with extent-ranked or explicitly selected geographic transformation and automatic or custom Z scaling (headless; `landxml` extra; also Tool 8.10a in the `.pyt` toolbox) |
...
| `autogis envmon trace-survey123` | CLOUD (base offline; `survey123` extra for live layers) | `core/envmon/submission_provenance.py` |
