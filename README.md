...
`manage-callout-overrides` subcommands are counted individually), and a **1687-test** arcpy-free
...
```bash
pip install -e ".[dev]"
python -m pytest -q           # 1687 tests (see: python -m pytest --collect-only -q)
```
...
Test baseline: **1687 tests** (`python -m pytest --collect-only -q`). All core logic is
