# -*- coding: utf-8 -*-
"""EnvironmentalMonitoringTools.pyt — ArcGIS Pro 3.5+ Python toolbox.

Ten tools per specification. All heavy lifting lives in ../src so the
logic is importable and (where arcpy-free) unit-testable.
"""
...
from qa_checks import QACollector  # noqa: E402
from envmon_config import (  # noqa: E402
    ParserProfile, SiteConfig, load_config, load_analyte_dictionary,
    load_screening_levels, FigureSpec,
)
...
class Toolbox(object):
    def __init__(self):
        self.label = "Environmental Monitoring Tools"
        self.alias = "envmon"
        self.tools = [
            InspectEnvironmentalWorkbook,
            ImportEnvironmentalWorkbook,
            BuildCurrentEventTables,
            BuildAnalyticalCallouts,
            BuildGroundwaterContours,
            ExportEnvironmentalFigures,
            RunFullEnvironmentalFigurePipeline,
            ValidateEnvironmentalDatabase,
            CreateParserProfileFromWorkbookDraft,
            CreateFigureSpecTemplate,
        ]


class InspectEnvironmentalWorkbook(object):
    def __init__(self):
    ...
    def execute(self, parameters, messages):
        from excel_workbook_inspector import (
...
class ImportEnvironmentalWorkbook(object):
    def __init__(self):
    ...
    def execute(self, parameters, messages):
        from import_to_gdb import run_import
...
class BuildCurrentEventTables(object):
    def __init__(self):
    ...
    def execute(self, parameters, messages):
        from build_current_event import build_current_event_wide
        qa = QACollector()
        p = {q.name: q for q in parameters}
        site = SiteConfig.load(Path(p["site_config"].valueAsText))
...
class BuildAnalyticalCallouts(object):
    def __init__(self):
    ...
    def execute(self, parameters, messages):
        from build_current_event import build_current_event_wide
        from build_figure_dataset import generate_callout_features
        qa = QACollector()
        p = {q.name: q for q in parameters}
        gdb = Path(p["gdb"].valueAsText)
        site = SiteConfig.load(Path(p["site_config"].valueAsText))
...
class BuildGroundwaterContours(object):
    def __init__(self):
    ...
    def execute(self, parameters, messages):
        from groundwater_contours import build_groundwater_contours
        qa = QACollector()
        p = {q.name: q for q in parameters}
        site = SiteConfig.load(Path(p["site_config"].valueAsText))
...
class ExportEnvironmentalFigures(object):
    def __init__(self):
    ...
    def execute(self, parameters, messages):
        from export_figures import export_layouts, register_exports
...
class RunFullEnvironmentalFigurePipeline(object):
    def __init__(self):
...
class ValidateEnvironmentalDatabase(object):
    def __init__(self):
...
class CreateParserProfileFromWorkbookDraft(object):
    def __init__(self):
...
class CreateFigureSpecTemplate(object):
    def __init__(self):
