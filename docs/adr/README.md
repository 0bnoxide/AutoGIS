...
| [110](0110-ci-and-agent-tooling-batch.md) | Claude Code workflow tooling batch (no product code): GitHub Actions CI running the arcpy-free suite on Windows (`windows-2022` pin added by ADR-0119; ubuntu unverified); `arcpy-doc-verifier` subagent enforcing the ADR-0077 dual check (valid at Pro 3.6 runtime, not deprecated at 3.5 floor) against `docs/arcpy-official-references.md`; `next_adr_number.py` preflight (local + open-PR scan, fail-soft) plus `coord reserve-adr` for atomic cross-session number reservation in the shared `claims.json`. Drafted as 0107, collided with PR #296, renumbered to a reserved 0110 | Accepted | 2026-07-23 |
...
| [114](0114-import-snapshot-hardening-salvage.md) | Import/snapshot hardening salvage from abandoned branch `claude/sleepy-wozniak-u2t1xn` — `validate_snapshot_id` rejects empty/whitespace/Windows-illegal `site_id`/`event_id` before `CreateFileGDB` (closes #219) and a `FileExistsError` pre-check replaces a silent same-day collision; `zero_rows_parsed` QA WARNING when `run_import` parses no rows (partially closes #220 — `qa_status` stays PASS by design; a zero-row `replace_*` run no longer deletes the event it replaces). Drops the original batch's `GW_ANALYTICAL` mirror (already on `main` via `normalize_groundwater.py`, #221 correctly closed) and its `"SOIL"`→`"SOIL_ANALYTICAL"` flip (now wrong — PR #337 made `SOIL` canonical). Renumbered from the original ADR-0079 | Accepted | 2026-07-26 |
| [115](0115-survey123-form-validation-and-schema-drift.md) | Survey123 Phase 1: form validation + schema drift — first XLSForm reader (`core/envmon/survey_schema.py`, openpyxl+stdlib; rejects workbooks with no `survey` sheet, case-insensitive sheet match); `envmon validate-survey-form` (QA contract, SampleID-contract divergence = ERROR, xlsform.org name rules — choice values are not identifiers, config cross-refs with visible `cross_checks_skipped`, builder/validator round-trip pin incl. leading-digit location) + `envmon diff-survey-schema` (safe/review-required/destructive taxonomy, exits 0/2/3, usage/IO = 1; feature-layer leg reuses `audit_schema.diff_schema` on the saved local-spec YAML; semantic exits log run-history success via `_SEMANTIC_EXIT_CODES`); Phase 1 gate met from saved artifacts, no portal, base install | Accepted | 2026-07-25 |
