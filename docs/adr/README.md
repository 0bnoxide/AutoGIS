...
| [113](0113-survey123-lifecycle-sampleid-contract.md) | Survey123 Phase 0 slice A: lifecycle SampleID contract — single owner module `core/envmon/sample_id.py` (`{location}-{YYYYMMDD}-{matrix}[-{qc}]`); planner/form/normalizer converge, `IsFieldDup` question makes `-FD` producible from the field (amends ADR-0021), structural QC guard stops a duplicate fuzzy-consuming its primary at difflib 0.914 ≥ 0.85; `sampling_plan`/`legacy_migrator` documented as non-lifecycle | Accepted | 2026-07-25 |
