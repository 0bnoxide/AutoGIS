...
| [114](0114-import-snapshot-hardening-salvage.md) | Import/snapshot hardening salvage from abandoned branch `claude/sleepy-wozniak-u2t1xn` — `validate_snapshot_id` rejects empty/whitespace/Windows-illegal `site_id`/`event_id` before `CreateFileGDB` (closes #219) and a `FileExistsError` pre-check replaces a silent same-day collision; `zero_rows_parsed` QA WARNING when `run_import` parses no rows (closes #220). Drops the original batch's `GW_ANALYTICAL` mirror (already on `main` via `normalize_groundwater.py`, #221 correctly closed) and its `"SOIL"`→`"SOIL_ANALYTICAL"` flip (now wrong — PR #337 made `SOIL` canonical). Renumbered from the original ADR-0079 | Accepted | 2026-07-26 |
