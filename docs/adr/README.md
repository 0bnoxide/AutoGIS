...
| [027](0027-import-rtk-survey-points.md) | ImportRTKSurveyPoints — configurable-column CSV parser + two-table GDB write; amended for issue #344 so `SiteID`/`BatchID` persist on both raw and QA rows and SCHEMA_VERSION advances 2.6→2.7 | Accepted (amended 2026-07-30) | 2026-06-28 |
...
| [110](0110-ci-and-agent-tooling-batch.md) | CI and agent tooling: arcpy-free pytest on pinned Windows 2022, SonarCloud CI analysis (Automatic Analysis disabled), arcpy-doc verifier, and collision-safe ADR numbering | Accepted | 2026-07-23 |
...
| [113](0113-survey123-lifecycle-sampleid-contract.md) | Survey123 Phase 0 slice A: lifecycle SampleID contract — single owner module `core/envmon/sample_id.py` (`{location}-{YYYYMMDD}-{matrix}[-{qc}]`); planner/form/normalizer converge; amended for issue #361 so form duplicates are distinct `field_dup_a`/`field_dup_b` → `-FD-A`/`-FD-B`, ambiguous selections fail, legacy `field_dup` → `-FD` remains readable, and duplicate-key loss is an ERROR; normalizer populates `IsDuplicate`/`DuplicateType`/`ParentSampleID`; QC-class guard closes #360 | Accepted (amended 2026-07-30) | 2026-07-25 |
| [114](0114-import-snapshot-hardening-salvage.md) | Import/snapshot hardening — snapshot ID validation closes #219; amended for issue #220 so unfiltered zero-row imports are blocking ERROR/FAIL (including `.pyt` ExecuteError), explicit empty matrix filters remain WARNING/PASS, and zero-row replace never deletes existing data | Accepted (amended 2026-07-30) | 2026-07-26 |
| [115](0115-survey123-form-validation-and-schema-drift.md) | Survey123 Phase 1: form validation + schema drift — first XLSForm reader; `validate-survey-form` enforces the SampleID contract and every `selected(...)` choice reference (`field_dup_a`/`field_dup_b` after #361); `diff-survey-schema` classifies safe/review-required/destructive changes with semantic exits; feature-layer leg reuses `audit_schema.diff_schema`; no portal, base install | Accepted (amended 2026-07-30) | 2026-07-25 |
| [116](0116-survey123-phase2-submission-sync.md) | Survey123 Phase 2 slice 1: incremental submission sync — one module owns the phase (`core/envmon/survey_sync.py`, arcgis-free at import, `fetch_item_pulls()` the single live seam); canonical `SubmissionEnvelope` (relocated here from ADR-0113 to be designed against its first consumer) carrying identity/GlobalID/operation/edit time/`repeat_path`/attachment metadata/canonical-JSON `payload_hash`/pull provenance; per-layer max-`EditDate` watermark with a strictly-greater where-clause so clean re-runs pull nothing; deletions via GlobalID sweep against the checkpoint; checkpoint written atomically only after staging artifacts land; `--since` replay never advances the checkpoint, `--dry-run` writes nothing; staging `envelopes_<ts>.jsonl` + `submissions_<ts>.csv` feed the existing `route-survey123` normalizer; new `survey123` extra, CLOUD and read-only (GDB writes stay downstream/LOCAL) | Proposed (owner sign-off pending) | 2026-07-25 |
...
| [123](0123-survey123-phase3-event-reconciliation.md) | Survey123 Phase 3: five-source event reconciliation — headless presence-matrix engine (`core/envmon/reconcile_event.py` + `envmon reconcile-event`) over plan/field/COC/lab/GDB; six-outcome taxonomy with fixed precedence (needs_review > orphan > not_collected > stalled > detail_conflict > reconciled); cascading anchor (plan optional per sample, D3); QC-class expected-presence masks (`QC_MASKS`) with per-sample `--presence-overrides`; exact-only matching (D7), fuzzy matches are non-consuming suggestions only (never cross QC class, never NODATE); documented CSV/JSON input contract (no persisted plan artifact or GDB exporter exists); `_SEMANTIC_EXIT_CODES` exit-2 convention (ADR-0115 defect class); corrects the design spec's §7 event-window claim (deferred, not shipped). Deferred: attachment-hash exceptions, per-record observation tracing (#414); limitation: no GDB-leg COC linkage until #420; owner-gated "sanitized real event reconciles end-to-end" sign-off leg | Proposed (owner sign-off pending) | 2026-08-02 |
