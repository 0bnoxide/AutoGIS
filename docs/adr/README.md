...
| [107](0107-ci-and-agent-tooling-batch.md) | Claude Code workflow tooling batch (no product code): GitHub Actions CI running the arcpy-free suite on `windows-latest` (repo is Windows-native; ubuntu unverified); `arcpy-doc-verifier` subagent enforcing the ADR-0077 dual check (valid at Pro 3.6 runtime, not deprecated at 3.5 floor) against `docs/arcpy-official-references.md`; `next_adr_number.py` preflight numbering ADRs from local files + open PRs (fail-soft). Number picked by the preflight; may renumber at merge | Proposed | 2026-07-23 |
