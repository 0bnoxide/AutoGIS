...
| [115](0115-survey123-form-validation-and-schema-drift.md) | Survey123 Phase 1: form validation + schema drift — first XLSForm reader (`core/envmon/survey_schema.py`, openpyxl+stdlib); `envmon validate-survey-form` (QA contract, SampleID-contract divergence = ERROR, config cross-refs, builder/validator round-trip pin) + `envmon diff-survey-schema` (safe/review-required/destructive taxonomy, exits 0/2/3; feature-layer leg reuses `audit_schema.diff_schema` on the saved local-spec YAML); Phase 1 gate met from saved artifacts, no portal, base install | Accepted | 2026-07-25 |
