...
| [111](0111-phase9-fieldmaps-sync-preflight.md) | Phase 9 slice 1: headless Field Maps sync preflight (Tool 7.5, `agol fieldmaps-preflight`) — pure checks over injected-gis seams reusing `audit_schema`/`sync_layer`; CLI-first per ADR-0039/0043 (owner struck the notebook option); local side from CSV/manifest snapshots, arcpy FGDB leg deferred to slice 2; gate run deferred to the #307 sandbox. `edits_where_clause` gains an `edit_field` kwarg | Proposed | 2026-07-24 |
