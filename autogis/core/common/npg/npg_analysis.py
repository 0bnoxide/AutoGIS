...
AutoGIS note: `nearest_polygon` has been excluded because its body calls
`_w_` from npg.npg_bool_hlp which is part of the npg ecosystem not
available in AutoGIS. All other functions are pure numpy and are retained
unchanged.
...
# EXCLUDED: from npg.npg_bool_hlp import _w_  — requires npg ecosystem
# nearest_polygon uses _w_; see EXCLUDED marker below.
...
def closest_n(a, N=3, ordered=True):
    ...
    coords, dist, n_array = n_near(a, N=N, ordered=ordered, return_all=True)
...
# ---- (2) near analysis
#
# EXCLUDED: nearest_polygon — requires _w_ from npg.npg_bool_hlp
# def nearest_polygon(pnts, polys):
#     # EXCLUDED: requires arcpy/npg ecosystem — use numpy_geom wrapper instead
#     pass
