...
def inspect_workbook_structure(workbook_path: Path,
    ...
    for name in wb_v.sheetnames:
        ...
        info = {
            ...
            "row_classes": {}, "title_rows": [], "header_rows": [],
            "units_rows": [], "screening_rows": [], "blank_rows": [],
            "data_start_row": None, "date_columns": [], "id_columns": [],
            "formula_cells": 0, "formula_cells_missing_cached": [],
            "formula_errors": [], "nonnumeric_tokens": set(),
            "unsafe_field_names": [],
        }
        scan_to = min(scan_rows, max_row)
        for r in range(1, scan_to + 1):
            cls = _classify_row(_row_values(ws, r, max_col))
        ...
        for c in range(1, max_col + 1):
            col_dates = sum(isinstance(ws.cell(row=r, column=c).value,
                                       (datetime, date))
                            for r in range(start, min(start + 15, max_row + 1)))
            if col_dates >= 2:
            ...
            for c in range(1, max_col + 1):
                ...
                if isinstance(v, str):
                    ...
                    if sv.upper() in FORMULA_ERRORS:
                        ...
                        info["nonnumeric_tokens"].add(sv)
        info["nonnumeric_tokens"] = sorted(info["nonnumeric_tokens"])
...
def propose_parser_profile(report: dict, profile_id: str = "DRAFT") -> dict:
...
def write_inspection_outputs(report: dict, json_path: Path,
        ...
        with csv_path.open("w", newline="", encoding="utf-8") as fh:
            ...
            for name, info in report["sheets"].items():
                for key in ("used_range", "title_rows", "header_rows",
                            ...
                            "unsafe_field_names"):
                    w.writerow([name, key, json.dumps(info[key], default=str)])
