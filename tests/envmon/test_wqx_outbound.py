...
from autogis.core.envmon.wqx_outbound import (
    COL_CHARACTERISTIC, COL_CONDITION, COL_DATUM, COL_LAT, COL_LIMIT_TYPE,
    COL_LIMIT_VALUE, COL_MEDIA, COL_RESULT, MonitoringLocation, WqxExportConfig,
    WqxSourceRow, map_to_wqx,
)
...
def test_cli_produces_three_files(tmp_path):
    ...
    write_records_csv([
        _src("Benzene", result=5.0, units="ug/L", sample_id="MW-1-A"),
        _src("Lead", result=2.0, units="ug/L", location_id="MW-9", sample_id="MW-9-A"),
    ], results, record_class=WqxSourceRow)
...
def test_cli_multi_event(tmp_path):
    e1 = tmp_path / "e1.csv"
    e2 = tmp_path / "e2.csv"
    write_records_csv([_src("Benzene", result=1.0, units="ug/L", sample_id="a")],
                      e1, record_class=WqxSourceRow)
    write_records_csv([_src("Toluene", result=2.0, units="ug/L", sample_id="b")],
                      e2, record_class=WqxSourceRow)
